...
import { LazyMotion, domAnimation, m, useCycle, AnimatePresence } from 'framer-motion'
...
export default function Header({ locations, route, navigation, preview }) {
  ...
  return (
    <header className={`${isOpen ? 'fixed' : 'absolute'} pt-5 px-5 mb-5 absolute z-50 w-full font-bold text-base md:text-4xl bg-white`}>
                      ...
                      if(location.title !== sharedState.title) {
                        return (
                          <div
                            ...
                            onClick={() => {handleLocationChange(location.title, locations), setIsActive()}} 
                            key={location.title}
                          >
                            {location.title}
                          </div>
        ...
        { preview && (<div className="w-full h-8 text-center text-xl bg-red-300">Preview Mode</div>) }
        </LazyMotion>
    </header>
  )
}